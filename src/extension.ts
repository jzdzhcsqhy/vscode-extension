...
async function callGptApi(prompt_msg: string): Promise<string> {
    ...
    const apiUrl = vscode.workspace.getConfiguration('cunami').get('apiUrl')  as string ;
    const bearerToken = vscode.workspace.getConfiguration('cunami').get('apikey');
    const modelname = vscode.workspace.getConfiguration('cunami').get('model') as string;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    let optimizeCode = vscode.commands.registerCommand('extension.optimizeCode', async () => {
        ...
        if (editor) {
            let optimizePrompt = vscode.workspace.getConfiguration('cunami').get<string>('optimizeCodePrompt');
	
            const code = editor.document.getText(editor.selection);
            const msg = optimizePrompt?.replace(/\$\{content\}/g, code) as string;
    ...
    let codeMaker = vscode.commands.registerCommand('extension.codeMaker', async () => {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
            let optimizePrompt = vscode.workspace.getConfiguration('cunami').get<string>('codeMakerPrompt');
